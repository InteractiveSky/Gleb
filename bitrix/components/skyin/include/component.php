...
$arParams['NAME'] = trim($arParams['NAME'])?"«".$arParams['NAME']."»":"включаемую область";
			...
			if ($bCanEdit)
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arMenu[] = array(
						...
						"ICON" => "panel-edit-php",
						"TEXT"=>GetMessage("main_comp_include_edit_php", Array("#NAME#" => $arParams['NAME'])),
						"TITLE" => GetMessage("MAIN_INCLUDE_AREA_EDIT_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR", Array("#NAME#" => $arParams['NAME'])),
					);
				}
				$arIcons = array(
					array(
						...
						"TITLE"=>GetMessage("main_comp_include_edit", Array("#NAME#" => $arParams['NAME'])),
						"ALT" => GetMessage("MAIN_INCLUDE_AREA_EDIT_".$arParams["AREA_FILE_SHOW"], Array("#NAME#" => $arParams['NAME'])),
						"MENU" => $arMenu,
					),
				);
			}
			...
			if ($sFilePath != $sFilePathTMP && $bCanAdd)
			{
				...
				if($USER->CanDoOperation('edit_php'))
				{
					$arMenu[] = array(
						...
						"ICON" => "panel-edit-php",
						"TEXT"	=> GetMessage("main_comp_include_add_php", Array("#NAME#" => $arParams['NAME'])),
						"TITLE" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR", Array("#NAME#" => $arParams['NAME'])),
					);
				}
				$arIcons[] = array(
					"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
					...
					"TITLE" => GetMessage("main_comp_include_add", Array("#NAME#" => $arParams['NAME'])),
					"ALT" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"], Array("#NAME#" => $arParams['NAME'])),
					"MENU" => $arMenu,
				);
			}
		}
		elseif ($bCanAdd)
		{
			...
			if($USER->CanDoOperation('edit_php'))
			{
				$arMenu[] = array(
					...
					"ICON" => "panel-edit-php",
					"TEXT" => GetMessage("main_comp_include_add1_php", Array("#NAME#" => $arParams['NAME'])),
					"TITLE" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"]."_NOEDITOR", Array("#NAME#" => $arParams['NAME'])),
				);
			}
			$arIcons = array(
				array(
					...
					"DEFAULT" => $APPLICATION->GetPublicShowMode() != 'configure',
					"ICON" => "bx-context-toolbar-create-icon",
					"TITLE" => GetMessage("main_comp_include_add1", Array("#NAME#" => $arParams['NAME'])),
					"ALT" => GetMessage("MAIN_INCLUDE_AREA_ADD_".$arParams["AREA_FILE_SHOW"], Array("#NAME#" => $arParams['NAME'])),
					"MENU" => $arMenu,
				),
			);
